 ...
 * @param {Number} hight 
 */
function quickSort(sortArray: number[], low?: number, hight?: number) {
  ...
  while (left < right) { // 从表的两端交替向中间扫描
      while (left < right && sortArray[right] >= pivot) right--;
      if (left < right) sortArray[left++] = sortArray[right];// 用比基准小的记录替换低位记录
      while (left < right && sortArray[left] < pivot) left++;
      if (left < right) sortArray[right--] = sortArray[left]; // 用比基准大的记录替换高位记录
  }
